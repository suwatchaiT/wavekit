  ...
  return <main className="tool-page">
    ...
    <div className="tool-shell"><aside><b>TOOL CATEGORIES</b>{categories.map(c=><div key={c}><span>{c}</span>{toolCatalog.filter(t=>t.category===c).map(t=><Link className={t.slug===slug?"active":""} href={`/tools/${t.slug}`} key={t.slug}>{t.name}</Link>)}</div>)}</aside>
      <article className="tool-main">
        ...
        <div className="tool-references"><b>References</b>{(references[slug]??(tool.category==="IP Calculator"?ipReferences:[])).map((ref,i)=><span key={ref.url}>{i>0&&<i>•</i>}<a href={ref.url} target="_blank" rel="noreferrer">{ref.label} ↗</a></span>)}</div>
        <ToolActions slug={slug}/><div id="calculator-workspace"><ToolCalculator key={slug} slug={slug}/></div><ToolExplainer slug={slug}/>
        <section className="technical-review"><div className="review-heading"><div><span className="eyebrow">TECHNICAL BASIS</span><h2>Scope and review</h2></div><b className={`review-status ${meta.status==="Standards-based"?"standard":"estimate"}`}>{meta.status}</b></div><dl><div><dt>Model or standard</dt><dd>{meta.standard}</dd></div><div><dt>Version</dt><dd>{meta.version}</dd></div><div><dt>Applicable range</dt><dd>{meta.range}</dd></div><div><dt>Assumptions</dt><dd>{meta.assumptions}</dd></div><div><dt>Technical owner</dt><dd>{meta.technicalOwner}</dd></div><div><dt>Review level</dt><dd>{meta.reviewLevel}</dd></div><div><dt>Last technical review</dt><dd>{meta.reviewed}</dd></div></dl><a href={reportUrl} target="_blank" rel="noreferrer">Report a problem on GitHub ↗</a></section>
