...
export default async function ToolPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="tool-page"><header><Link className="brand" href="/"><span className="brand-mark">W</span><span>Wave<span>Kit</span><small>RF ENGINEERING TOOLS</small></span></Link><span className="version-badge">BETA 0.1</span><nav><Link href="/#tools">All tools</Link><Link href="/#reference">Technology</Link><Link href="/info/methodology">Methodology</Link></nav><Link className="header-cta" href="/">← Home</Link></header>
    <div className="tool-shell"><aside><b>TOOL CATEGORIES</b>{categories.map(c=><div key={c}><span>{c}</span>{toolCatalog.filter(t=>t.category===c).map(t=><Link className={t.slug===slug?"active":""} href={`/tools/${t.slug}`} key={t.slug}>{t.name}</Link>)}</div>)}</aside>
      <article className="tool-main"><div className="breadcrumbs"><Link href="/">Home</Link><i>›</i><span>{tool.category}</span><i>›</i><b>{tool.name}</b></div><span className="eyebrow">{tool.category.toUpperCase()}</span><h1>{tool.name}</h1><p className="tool-lead">{tool.summary} All calculations run locally in your browser.</p><div className="tool-references"><b>References</b>{(references[slug]??(tool.category==="IP Calculator"?ipReferences:[])).map((ref,i)=><span key={ref.url}>{i>0&&<i>•</i>}<a href={ref.url} target="_blank" rel="noreferrer">{ref.label} ↗</a></span>)}</div><ToolActions slug={slug}/><div id="calculator-workspace"><ToolCalculator key={slug} slug={slug}/></div><ToolExplainer slug={slug}/><section className="technical-review"><div className="review-heading"><div><span className="eyebrow">TECHNICAL BASIS</span><h2>Scope and review</h2></div><b className={`review-status ${meta.status==="Standards-based"?"standard":"estimate"}`}>{meta.status}</b></div><dl><div><dt>Model or standard</dt><dd>{meta.standard}</dd></div><div><dt>Version</dt><dd>{meta.version}</dd></div><div><dt>Applicable range</dt><dd>{meta.range}</dd></div><div><dt>Assumptions</dt><dd>{meta.assumptions}</dd></div><div><dt>Last technical review</dt><dd>{meta.reviewed}</dd></div></dl><a href={`mailto:?subject=${encodeURIComponent(`WaveKit problem: ${tool.name}`)}&body=${encodeURIComponent(`Tool: ${tool.name}\nPage: /tools/${slug}\n\nProblem:\n\nExpected result:\n`)}`}>Report a problem ↗</a></section><section className="formula-note disclaimer-note"><h2>Reference-only disclaimer</h2><p>WaveKit is provided for learning, planning estimates and cross-checks. Do not use this output as the sole basis for a safety, regulatory, commercial, commissioning or real-world operational claim. Verify inputs and results against current standards, vendor documentation, calibrated tools and applicable local requirements.</p><Link href="/info/disclaimer">Read the full disclaimer →</Link></section></article>
    </div></main>
}
