...
export default async function ToolPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="tool-page">
    <header><a className="brand" href="/"><span className="brand-mark">W</span><span>Wave<span>Kit</span><small>RF ENGINEERING TOOLS</small></span></a><span className="version-badge">BETA 0.1</span><nav><a href="/#tools">All tools</a><a href="/#reference">Technology</a><a href="/info/methodology">Methodology</a><a href="/info/readiness">Readiness</a></nav><a className="header-cta" href="/">← Home</a></header>
    <div className="tool-shell"><aside><b>TOOL CATEGORIES</b>{categories.map(c=><div key={c}><span>{c}</span>{toolCatalog.filter(t=>t.category===c).map(t=><a className={t.slug===slug?"active":""} href={`/tools/${t.slug}`} key={t.slug}>{t.name}</a>)}</div>)}</aside>
      <article className="tool-main">
        <div className="breadcrumbs"><a href="/">Home</a><i>›</i><span>{tool.category}</span><i>›</i><b>{tool.name}</b></div><span className="eyebrow">{tool.category.toUpperCase()}</span><h1>{tool.name}</h1><p className="tool-lead">{tool.summary} All calculations run locally in your browser.</p>
        <div className="tool-references"><b>References</b>{mappedSpecs.map((item,i)=><span key={item!.number}>{i>0&&<i>•</i>}<a href={`/3gpp/spec/${item!.number}`}>{item!.type} {item!.number} — {item!.title} →</a></span>)}{externalReferences.map((ref,i)=><span key={ref.url}>{(i>0||mappedSpecs.length>0)&&<i>•</i>}<a href={ref.url} target="_blank" rel="noreferrer">{ref.label} ↗</a></span>)}</div>
        <ToolActions slug={slug}/><div id="calculator-workspace"><ToolCalculator key={slug} slug={slug}/></div><ToolExplainer slug={slug}/>
        <section className="technical-review"><div className="review-heading"><div><span className="eyebrow">TECHNICAL BASIS</span><h2>Scope and review</h2></div><b className={`review-status ${meta.status==="Standards-based"?"standard":"estimate"}`}>{meta.status}</b></div><dl>{relation&&<><div><dt>3GPP technology</dt><dd>{relation.technologies.join(" · ")}</dd></div><div><dt>Mapped features</dt><dd className="technical-links">{mappedFeatures.map(feature=><a href={`/3gpp?feature=${feature!.id}`} key={feature!.id}>{feature!.name} ({feature!.release}) →</a>)}</dd></div><div><dt>Release coverage</dt><dd>{relation.releases.join(" · ")}</dd></div><div><dt>Mapped specifications</dt><dd className="technical-links">{mappedSpecs.map(item=><a href={`/3gpp/spec/${item!.number}`} key={item!.number}>{item!.type} {item!.number} →</a>)}</dd></div></>}<div><dt>Model or standard</dt><dd>{meta.standard}</dd></div><div><dt>Version</dt><dd>{meta.version}</dd></div><div><dt>Applicable range</dt><dd>{meta.range}</dd></div><div><dt>Assumptions</dt><dd>{meta.assumptions}</dd></div><div><dt>Technical owner</dt><dd>{meta.technicalOwner}</dd></div><div><dt>Review level</dt><dd>{meta.reviewLevel}</dd></div><div><dt>Last technical review</dt><dd>{meta.reviewed}</dd></div></dl>{relation&&<a className="technical-map-link" href="/3gpp">Explore the complete 3GPP knowledge map →</a>}<a href={reportUrl} target="_blank" rel="noreferrer">Report a problem on GitHub ↗</a></section>
        <section className="formula-note disclaimer-note"><h2>Reference-only disclaimer</h2><p>WaveKit is provided for learning, planning estimates and cross-checks. Do not use this output as the sole basis for a safety, regulatory, commercial, commissioning or real-world operational claim. Verify inputs and results against current standards, vendor documentation, calibrated tools and applicable local requirements.</p><a href="/info/disclaimer">Read the full disclaimer →</a></section>
      </article>
    </div>
  </main>
}
