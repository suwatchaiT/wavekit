...
export default function KnowledgeExplorer({initialFeature}:{initialFeature?:string}) {
  ...
  return <>
    <section className="gpp-hero"><div><span className="eyebrow">3GPP KNOWLEDGE MAP</span><h1>Follow radio technology<br/><em>through every release.</em></h1><p>A searchable map connecting releases, features and the specifications engineers use. Official catalogue relationships stay separate from WaveKit’s curated engineering links.</p><div className="gpp-actions"><a className="primary" href="/3gpp/specifications">Browse {specifications.length} sample specifications →</a><span>Snapshot {snapshot.date}</span></div></div><aside><b>Prototype scope</b><strong>{features.length}</strong><span>curated features</span><strong>{specifications.length}</strong><span>representative specifications</span><p>The public catalogue import comes after local approval.</p></aside></section>
