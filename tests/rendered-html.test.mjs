...
test("renders public information pages",async()=>{for(const slug of ["about","methodology","privacy","disclaimer"]){const response=await render(`/info/${slug}`);assert.equal(response.status,200);const html=await response.text();assert.match(html,/WaveKit/);assert.match(html,/BETA 0\.1/)}});
